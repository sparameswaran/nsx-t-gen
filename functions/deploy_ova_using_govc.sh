...
function deploy_ova_using_govc() {
  type_of_ova=$1
  path_to_ova=$2
...
function deploy_ova {
  ...
  if [ "$resource_pool" == "" -o -z "$resource_pool" ]; then
    ...
    set +e
    found_rp=$(govc find . -name "${resource_pool}" -type p 2>&1 | grep Resources | grep "${resource_pool}$" || true )
...
(.PropertyMapping[] | select(.Key == "nsx_netmask_0")).Value = $netmask |
...
.NetworkMapping[3].Network = $mgmt_network |
.PowerOn = true |
.WaitForIP = true |
.Deployment = $deployment_size |
(.PropertyMapping[] | select(.Key == "nsx_hostname")).Value = $hostname |
